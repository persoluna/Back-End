        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between mb-4 space-y-4 sm:space-y-0">
                <div class="w-full sm:w-1/2">
                    <input wire:model.live.debounce.500ms="search" type="text" placeholder="Search Gallery Category..."
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full leading-normal mt-8">
                    <thead>
                        <tr>
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-left text-xs sm:text-sm font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                                Name
                            </th>
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-left text-xs sm:text-sm font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                    ...
                    <tbody>
                        @forelse ($gallerycategories as $gallerycategory)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-300">
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 flex text-sm gap-3">
                                    <a href="{{ route('gallerycategories.edit', $gallerycategory->id) }}"
                                    ...
                                    <form action="{{ route('gallerycategories.destroy', $gallerycategory->id) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            ...
                                            onclick="return confirm('Are you sure you want to delete this gallery category?')">Delete</button>
                                    </form>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm">
                                    <label class="inline-flex items-center cursor-pointer">
                                        <input type="checkbox" wire:model="gallerycategory.status"
                                            wire:change="updateGalleryCategoryStatus({{ $gallerycategory->id }})" class="sr-only peer"
                                            {{ $gallerycategory->status ? 'checked' : '' }}>
                                        <div
                                        ...
                                        <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $gallerycategory->status ? 'Active' : 'Inactive' }}</span>
                                    </label>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="text-center py-10 text-gray-500 dark:text-gray-400">
                                    ...
                                    <img src="{{ asset('storage/DrawKit-onlineshopping-Illustration-10.svg') }}" alt="No category found" class="mx-auto h-40 w-40 sm:h-60 sm:w-60">
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
