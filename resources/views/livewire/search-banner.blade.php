<div>
    <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex space-x-2">
            <a href="{{ route('banners.export') }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded inline-flex items-center">
    ...
    <table class="min-w-full leading-normal mt-8">
        <thead>
            <tr>
                <th class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-nowrap text-left text-[13px] sm:text-[15px] font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider dark:bg-slate-600">
                    Title
                </th>
                <th class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-nowrap text-left text-[13px] sm:text-[15px] font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider dark:bg-slate-600">
                    Banner Image
                </th>
                <th class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-nowrap text-left text-[13px] sm:text-[15px] font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider dark:bg-slate-600">
                    Alt Tag
            ...
            @forelse ($banners as $banner)
                <tr class="font-medium">
                    ...
                    <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm">
                        <img src="{{ asset('storage/banners/'. $banner->banner_image) }}" alt="{{ $banner->alt_tag }}" class="pt-4 max-w-full h-14 object-cover object-center">
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-300">
                        {{ $banner->alt_tag }}
                    </td>
                    <td class="px-5 py-8 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm flex gap-3">
                        <a href="{{ route('banners.show', $banner->id) }}" class="rounded-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">View</a>
                        <a href="{{ route('banners.edit', $banner->id) }}" class="rounded-full bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Edit</a>
                        <form action="{{ route('banners.destroy', $banner->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="rounded-full bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onclick="return confirm('Are you sure you want to delete this banner?')">Delete</button>
                        </form>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm">
