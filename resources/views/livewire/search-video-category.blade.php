        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between mb-4 space-y-4 sm:space-y-0">
                <div class="w-full sm:w-1/2">
                    <input wire:model.live.debounce.500ms="search" type="text" placeholder="Search Video Category..."
                    ...
                    <tbody>
                        @forelse ($videocategories as $videocategory)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-300">
                                <td class="truncate px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-300">
                                    {{ $videocategory->name }}
                                </td>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm">
                                    <div class="flex flex-col sm:flex-row gap-2">
                                        ...
                                        <form action="{{ route('videocategories.destroy', $videocategory->id) }}" method="POST"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                ...
                                                onclick="return confirm('Are you sure you want to delete this videocategory?')">Delete</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="text-center py-10 text-gray-500 dark:text-gray-400">
                                    ...
                                    <img src="{{ asset('storage/DrawKit-onlineshopping-Illustration-10.svg') }}" alt="No video category found" class="mx-auto h-40 w-40 sm:h-60 sm:w-60">
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
