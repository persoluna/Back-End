<div>
    <div>
        <input wire:model.live.debounce.500ms="search" type="text" placeholder="Search qualitycontrols..."
            ...
            @forelse ($qualitycontrols as $qualitycontrol)
                <tr class="font-medium">
                    ...
                    <td class="px-5 py-8 border-b border-gray-200 bg-white flex text-sm gap-3">
                        ...
                        <form action="{{ route('qualitycontrols.destroy', $qualitycontrol->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="rounded-full bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                                onclick="return confirm('Are you sure you want to delete this qualitycontrol ?')">Delete</button>
                        </form>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="3" class="text-center text-gray-500 font-bold">No qualitycontrols found for
                        "{{ $search }}"!</td>
                </tr>
            @endforelse
        </tbody>
