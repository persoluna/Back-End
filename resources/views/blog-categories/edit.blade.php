<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 mb-8">
            ...
            <nav class="flex items-center space-x-4">
                @if (isset($blogcategory))
                    ...
                    <span class="text-white">/</span>
                    <a href="{{ route('blogcategories.edit', $blogcategory->id) }}" class="text-white hover:text-gray-200">{{ $blogcategory->name }}</a>
                @else
                    <a href="{{ route('dashboard') }}" class="text-white hover:text-gray-200">Home</a>
                @endif
            </nav>
        ...
        <!-- Category form -->
        <form action="{{ route('blogcategories.update', $blogcategory->id) }}" method="POST" class="w-full"
