<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 mb-8">
            <h1 class="text-4xl font-bold text-white mb-4">Edit Video Category</h1>
            <nav class="flex items-center space-x-4">
                ...
                @if (isset($videocategory))
                    <span class="text-white">/</span>
                    <a href="{{ route('videocategories.edit', $videocategory->id) }}" class="text-white hover:text-gray-200">{{ $videocategory->name }}</a>
                @endif
            </nav>
        </header>
        ...
        <!-- Category form -->
        <form action="{{ route('videocategories.update', $videocategory->id) }}" method="POST" class="w-full"
