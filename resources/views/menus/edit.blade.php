    ...
    <div class="bg-white min-h-[800px] pt-12">
        @if (isset($menu))
            <x-breadcrumb :breadcrumbs="[
                ...
                ['name' => $menu->page_name, 'url' => route('menus.edit', $menu->id)],
            ]" />
        @else
            <x-breadcrumb :breadcrumbs="[
                ['name' => 'Home', 'url' => route('dashboard')],
                ['name' => 'Page', 'url' => route('menus.index')],
            ]" />
        @endif

        <!-- Page edit title -->
        <div class="mb-8 space-y-3">
            <h1 class="text-3xl font-semibold text-center sm:text-left sm:pl-[80px] pt-[90px]">Edit Page</h1>
        </div>

        <!-- Page form -->
        <form action="{{ route('menus.update', $menu->id) }}" method="POST" class="w-full" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-10 items-center grid lg:grid-cols-2 gap-6 m-[80px] justify-center">
                ...
                <div class="lg:col-span-1">
                    ...
                    <input
                        ...
                        value="{{ old('page_name', $menu->page_name) }}">
                    @error('page_name')
                ...
                <div class="lg:col-span-1">
                    ...
                    <div id="image-container">
                        @if ($menu->banner_image)
                            <img src="{{ asset('storage/menu_banners/' . $menu->banner_image) }}"
                                alt="{{ $menu->alt_tag }}" class="min-h-[100px] w-auto p-2">
                        @endif
                    </div>
                ...
                <div class="lg:col-span-1">
                    <label
                        class="text-base font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                        for="alt_tag">Banner Image Alt Tag</label>
                    <input
                        class="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-10 w-full rounded-md border px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        id="alt_tag" placeholder="Enter the alt tag for the Banner Image" type="text"
                        name="alt_tag" value="{{ old('alt_tag', $menu->alt_tag) }}">
                    @error('alt_tag')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
                        </div>
                ...
                <div class="lg:col-span-1">
                    ...
                    <input
                        ...
                        name="meta_description" value="{{ old('meta_description', $menu->meta_description) }}">
                ...
                <div class="lg:col-span-1">
                    ...
                    <input
                        ...
                        value="{{ old('meta_canonical', $menu->meta_canonical) }}">
