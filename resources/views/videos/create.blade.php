<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 mb-8">
            <h1 class="text-4xl font-bold text-white mb-4">Create Your Video</h1>
            <nav class="flex items-center space-x-4">
                ...
                <span class="text-white">/</span>
                <a href="{{ route('videos.create') }}" class="text-white hover:text-gray-200">Create Video</a>
            </nav>
        </header>

        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
            <div class="mb-8 space-y-3">
                <p class="text-gray-500 dark:text-gray-400 text-center text-xl">Enter the Video details below.</p>
            </div>
            <!-- Video form -->
            <form id="videoForm" action="{{ route('videos.store') }}" method="POST" class="w-full" enctype="multipart/form-data">
                @csrf
                <div class="mb-10 items-center grid lg:grid-cols-2 gap-6 justify-center">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea
                            ...
                            placeholder="Enter a brief description of your video">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="text-red-500 mt-2 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="lg:col-span-1">
                        <label for="video" class="text-base font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Video File</label>
                        <input
                            id="video"
                            name="video"
                            type="file"
