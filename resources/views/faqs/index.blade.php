<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header id="sticky-header" class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 mb-8 sticky top-16 transition-all duration-300 ease-in-out z-10">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-4xl font-bold text-white mb-4">FAQs</h1>
                    <nav class="flex items-center space-x-4">
                        <a href="{{ route('dashboard') }}" class="text-white hover:text-gray-200">Home</a>
                        <span class="text-white">/</span>
                        <a href="{{ route('faqs.index') }}" class="text-white hover:text-gray-200">FAQs</a>
                    </nav>
                </div>
                <a href="{{ route('faqs.create') }}" class="inline-block px-6 py-3 bg-white text-indigo-600 font-semibold rounded-lg shadow-md hover:bg-indigo-600 hover:text-white transform hover:-translate-y-0.5 transition duration-300 ease-in-out">
                    Create FAQ
                </a>
            </div>
        </header>
    ...
    <script>
        ...
        const toggleCheckbox = document.querySelector('input[wire:model="faq.status"]');

        toggleCheckbox.addEventListener('change', function() {
            if (this.checked) {
                toggleMessage.textContent = 'FAQ status changed to Active.';
                toggleMessage.classList.remove('hidden');
            } else {
                toggleMessage.textContent = 'FAQ status changed to Inactive.';
                toggleMessage.classList.remove('hidden');
            }
