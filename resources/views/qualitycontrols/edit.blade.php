    ...
    <div class="bg-white pt-12 pb-12">
        @if (isset($qualitycontrol))
            <x-breadcrumb :breadcrumbs="[
                ['name' => 'Qualitycontrols', 'url' => route('qualitycontrols.index')],
                ['name' => $qualitycontrol->title, 'url' => route('qualitycontrols.edit', $qualitycontrol->id)],
            ]" />
        @else
            <x-breadcrumb :breadcrumbs="[
        ...
        <div class="mb-8 space-y-3">
            <h1 class="text-3xl font-semibold text-center sm:text-left sm:pl-[80px] pt-[90px]">Edit Qualitycontrol</h1>
        </div>

        <!-- form -->
        <form action="{{ route('qualitycontrols.update', $qualitycontrol->id) }}" method="POST" class="w-full" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-10 items-center grid lg:grid-cols-2 gap-6 m-[80px] justify-center">
                ...
                <div class="lg:col-span-1">
                    ...
                    <textarea
                        ...
                        id="description" placeholder="Enter the description" type="text"
                        name="description">{{ old('description', $qualitycontrol->description) }}</textarea>
                    @error('description')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
                        </div>
