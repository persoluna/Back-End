<x-app-layout>
    <div class="grid sm:grid-cols-6 overflow-hidden">
                                ...
                                <div class="overflow-hidden">
                                    ...
                                    <table class="min-w-full">
                                        <thead class="bg-gray-50 dark:bg-gray-700">
                                            <tr>
                                                <th scope="col" class="p-3 text-xl font-medium tracking-wider text-left text-gray-700 dark:text-gray-400">
                                                    Browser
                                                </th>
                                                <th scope="col" class="p-3 text-xl font-medium tracking-wider text-left text-gray-700 dark:text-gray-400">
                                                    Page Views
                                            ...
                                            @foreach($topBrowsers as $browser)
                                                <tr class="bg-white border-b border-dashed dark:bg-gray-800 dark:border-gray-700">
                                                    <td class="p-3 text-xl font-medium whitespace-nowrap dark:text-white">
                                                        <img src="{{ asset('storage/icons/' . strtolower($browser['browser']) . '.svg') }}" alt="{{ $browser['browser'] }}" class="mr-2 h-10 inline-block">{{ $browser['browser'] }}
                                                    </td>
        ...
        <div class="col-span-6 w-screen sm:w-full pt-10 pl-2 pr-3">
            <div class="relative overflow-x-auto shadow-md dark:shadow-lg-dark sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <caption class="p-5 text-3xl font-semibold text-left rtl:text-right text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                        Popular Pages
                        <p class="mt-1 text-xl font-normal text-gray-500 dark:text-gray-400">
                            Browse the list of most visited pages of the website.
                        </p>
                    </caption>
                    ...
                    <tbody>
                        @foreach ($getVisitorsAndPageViews as $data)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ Str::limit($data['pageTitle'], 80) }}
                                </th>
                                <td class="px-6 py-4">{{ Str::limit($data['activeUsers'], 10) }}</td>
                                <td class="px-6 py-4">{{ Str::limit($data['screenPageViews'], 10) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="col-span-6 sm:col-span-4 w-fit pt-12 pl-2 pb-4">
             <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-lg-dark p-6">
                <h5 class="text-xl font-bold mb-2 text-gray-900 dark:text-white">Top Operating system</h5>
                <p class="text-gray-600 dark:text-gray-400 mb-4">Number of users sorted by Operating system</p>
    ...
    <script>
        ...
        function addEventListeners() {
            document.querySelectorAll(".allPaths").forEach(e => {
                e.addEventListener("mouseover", function () {
                    ...
                    const countryName = e.id;
                    const userCount = userCounts[countryName] || 0;
                    document.getElementById("namep").innerText = `${countryName} (${userCount})`;
                });
        ...
        function updateCountryColor(element) {
            const countryName = element.id;
            const userCount = userCounts[countryName] || 0;
