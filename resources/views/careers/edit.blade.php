<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 mb-8">
            <h1 class="text-4xl font-bold text-white mb-4">Edit Career</h1>
            <nav class="flex items-center space-x-4">
                ...
                @if (isset($career))
                    <span class="text-white">/</span>
                    <a href="{{ route('careers.edit', $career->id) }}" class="text-white hover:text-gray-200">{{ $career->title }}</a>
                @endif
            </nav>
        </header>
        ...
        <!-- career form -->
        <form id="editCareerForm" action="{{ route('careers.update', $career->id) }}" method="POST" class="w-full" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-10 items-center grid lg:grid-cols-2 gap-6 m-[80px] justify-center">

                <!-- career Title input field -->
                <div class="lg:col-span-1">
                    <label
                        class="text-base font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                        for="title">Career Title</label>
                    <input
                ...
                <div class="lg:col-span-1">
                    <label
                        class="text-base font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                        for="requirement">Career Requirement</label>
                    <input
                        class="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-10 w-full rounded-md border px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                ...
                <div class="lg:col-span-1">
                    ...
                    <textarea
                        ...
                        id="brief_description" placeholder="Enter the brief description" type="text"
                        name="brief_description">{{ old('brief_description', $career->brief_description) }}</textarea>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea
                        class="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-20 w-full rounded-md border px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        id="editor" placeholder="Enter the description" name="description">{{ old('description', $career->description) }}</textarea>
                    @error('description')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
                        </div>
    ...
    <script>
        ...
        document.getElementById('editCareerForm').addEventListener('submit', function(event) {
        ...
        document.getElementById('leave-page').addEventListener('click', function() {
            ...
            window.location.href = '{{ route('careers.index') }}'; // Redirect to careers index
        });
