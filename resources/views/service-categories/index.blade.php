<x-app-layout>
    <div class="note-container py-12">
        <x-breadcrumb :breadcrumbs="[['name' => 'Service Categories', 'url' => route('servicecategories.index')]]" />
        <div class="m-4 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow rounded-lg overflow-hidden flex-col">
                <div class="grid gap-6 justify-start sm:justify-between lg:grid-cols-1 items-center mb-4">
                    <h1 class="sm:text-4xl text-3xl font-semibold  sm:pb-4 pb-8">Service Categories</h1>
                ...
                <script>
                    ...
                    const toggleCheckbox = document.querySelector(
                        'input[wire:model="servicecategory.status"]');

                    toggleCheckbox.addEventListener('change', function() {
                        if (this.checked) {
                            toggleMessage.textContent = 'servicecategory status changed to Active.';
                            toggleMessage.classList.remove('hidden'); // Show the message
                        } else {
                            toggleMessage.textContent = 'servicecategory status changed to Inactive.';
                            toggleMessage.classList.remove('hidden'); // Show the message
                        }
                ...
                {{-- ! Live wire search for Service Category --}}
                <div class="py-12">
                    @livewire('search-service-category')
                </div>
            </div>
        </div>
    </div>
